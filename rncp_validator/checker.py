...
def get_all_commits(git_path: str) -> dict:
    """
    ...
    repo = Repo(git_path)
    print(f"Working on all commits from {repo.active_branch.name}...")
    for commit in repo.iter_commits(repo.active_branch.name):
